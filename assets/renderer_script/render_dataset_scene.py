...
class Render:
    def __init__(self):
        ...
        ## camera pivot
        self.axis = bpy.data.objects['Camera_Arm']
        self.light_1 = bpy.data.objects['Light1']
        self.light_2 = bpy.data.objects['Light2']
        ## bottles
        self.obj_names = ['beer_golden', 'coca_cola_can', 'koicha', 'monster_can', 'nihon_no_koohii',
                          'ocha_can_500ml', 'pepsi_large', 'waterbottle_6_new', 'waterbottle_large', 'aluminium_can']
        self.objects = self.create_objects() # Create list of bpy.data.objects from bpy.data.objects[1] to bpy.data.objects[N]

        ## Render information
        self.camera_d_limits = [-16, -14] #[0.2, 0.8] # Define range of zoom  in m that the camera is going to pan through
        self.beta_limits = [0,30] #[80, -80] # Define range of beta angles that the camera is going to pan through
        ...
        self.images_filepath = 'assets/dataset_results'
        self.labels_filepath = 'assets/dataset_results/labels/'

    def set_camera(self):
        self.axis.rotation_euler = (0, 0, 0)
        self.axis.location = (0, 0, 0)
        self.camera.location = (0, -16, 8.5)
    ...
    def main_rendering_loop(self, rot_step):
        ...
        if accept_render == 'Y': # If the user inputs 'Y' then procede with the data generation
            ...
            dmin = self.camera_d_limits[0]#int(self.camera_d_limits[0] * 10)
            dmax = self.camera_d_limits[1]#int(self.camera_d_limits[1] * 10)
            ...
            # Begin nested loops
            for d in range(dmin, dmax + 1, 1): # Loop to vary the height of the camera
                ## Update the height of the camera
                self.camera.location = (0, d, 8.5) # Divide the distance z by 10 to re-factor current height

                # Refactor the beta limits for them to be in a range from 0 to 360 to adapt the limits to the for loop
                min_beta = self.beta_limits[0]#(-1)*self.beta_limits[0] + 90
                max_beta = self.beta_limits[1]#(-1)*self.beta_limits[1] + 90

                for beta in range(min_beta, max_beta + 1, rotation_step): # Loop to vary the angle beta
                    beta_r = beta#(-1)*beta + 90 # Re-factor the current beta
                    ...
                    for gamma in range(self.gamma_limits[0], self.gamma_limits[1] + 1, rotation_step): # Loop to vary the angle gamma
                        ...
                        #axis_rotation = (0, 0, m.radians(gamma)) 
                        ...
                        ## Configure lighting
                        energy1 = random.randint(50, 150) # Grab random light intensity
                        self.light_1.data.energy = energy1 # Update the <bpy.data.objects['Light']> energy information
                        energy2 = random.randint(50, 150) # Grab random light intensity
    ...
    def get_all_coordinates(self):
        ...
        for i, objct in enumerate(self.objects): # Loop through all of the objects
            ...
            if b_box: # If find_bounding_box() doesn't return None
                ...
                ## change the labels into cans and bottles
                if i in [0,1,3,5,9]: #['beer_golden', 'coca_cola_can', 'monster_can', 'ocha_can_500ml',  'aluminium_can']
                    lb = 1
                else:
                    lb = 0 

                text_coordinates = self.format_coordinates(b_box, lb) # Reformat coordinates to YOLOv3 format
    ...
    def render_blender(self, count_f_name):
        ...
        self.xpix = 1920#random.randint(500, 1000)
        self.ypix = 1080#random.randint(500, 1000)
