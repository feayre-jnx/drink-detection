...
# Initiate model
model = ObjectDetectionModel(opt.model_config_path)
...
for batch_i, (_, imgs, targets) in enumerate(tqdm.tqdm(dataloader, desc="Detecting objects")):
    ...
    output = []
    with torch.no_grad():
        cls_preds, conf_preds, bbox_preds = model(imgs)
        for i in range(len(cls_preds)):
            output.append(torch.cat((bbox_preds[i], conf_preds[i], cls_preds[i]), -1))

        detections = torch.cat(output, 1)
        detections = detections.cpu()
        outputs = non_max_suppression(detections, 80, opt.conf_thres, opt.nms_thres)
